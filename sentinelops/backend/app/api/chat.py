...
Uses OpenRouter (OpenAI-compatible) LLM with a rich system prompt.
"""
from fastapi import APIRouter, HTTPException
from pydantic import BaseModel
from app.core.openai_client import openai_chat
...
SYSTEM_PROMPT = """You are SentinelOps, an industrial AI maintenance assistant.
You are monitoring Jurong Plant A, a precision manufacturing facility in Singapore.
You have four AI sub-agents: Anomaly Detector, Fault Classifier, Predictive Maintenance Agent, and yourself (Orchestrator).
...
async def chat(request: ChatRequest) -> ChatResponse:
    ...
    # openai_chat() takes one `prompt` + one `system`, so we embed prior
    # turns into the prompt the same way the Pangu version did.
    ...
    try:
        ...
        # Surface LLM errors to the frontend instead of a silent 500
