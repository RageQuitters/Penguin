...
-------------------------------
Coordinates all sub-agents with conditional routing. Uses Pangu LLM
(Huawei Cloud) to justify decisions and synthesise the final work order.
...
Flow:
  ...
  5. Pangu synthesises all results into a natural-language work order
"""
import json
...
async def run_orchestrator(machine_id: str, reading: SensorReading, trace: TraceLog) -> OrchestratorDecision:
    ...
    if score < 0.4:
        skip_reason = await _llm_justify_skip(machine_id, score)
...
async def _build_final(
    ...
    trace.add_entry(agent=AGENT_NAME, action=TraceAction.REASONING,
        ...
        explanation="Sending all agent results to Pangu LLM for final synthesis and work order generation")

    llm = await _llm_synthesise(machine_id, reading, anomaly, fault, predictive, routing_reason)

    urgency = Urgency(llm.get("overall_urgency", "low"))
    ...
    decision = OrchestratorDecision(
        ...
        work_order=llm.get("work_order", "No action required. Continue monitoring."),
        agents_called=agents_called,
        anomaly=anomaly, fault=fault, predictive=predictive,
    )

    trace.add_entry(agent=AGENT_NAME, action=TraceAction.DECISION,
        output_data={"status": status.value, "urgency": urgency.value, "work_order": decision.work_order, "agents": agents_called},
        explanation=f"FINAL — Status: {status.value} | Urgency: {urgency.value} | Work order issued.")

    return decision


async def _llm_synthesise(machine_id, reading, anomaly, fault, predictive, routing_reason) -> dict:
...
async def _llm_justify_skip(machine_id: str, score: float) -> str:
    raw = await pangu_chat(
        f"Machine {machine_id} anomaly score is {score:.4f}, below the 0.4 alert threshold. "
        f"In one sentence, state why no further agent analysis is needed and what the recommended monitoring action is.",
        max_tokens=100,
    )
    return raw.strip()
