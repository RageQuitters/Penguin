...
export default function SentinelChat({ analyzeResult }) {
  ...
  // When a new analysis completes, inject TinyLlama's final work order into the chat.
  // Track the last session_id so we only fire once per analysis run.
  ...
  useEffect(() => {
    ...
    const decision = analyzeResult.decision;
    if (!decision) return;

    const machineId = analyzeResult.machine_id;
    const urgency = decision.overall_urgency ?? 'unknown';
    const status = decision.final_status ?? 'unknown';
    const workOrder = decision.work_order ?? 'No work order generated.';
    const faults = decision.fault?.active_faults ?? [];
    const score = decision.anomaly?.anomaly_score;
    const rul = decision.predictive?.rul_hours;
    ...
    const lines = [
      `**Analysis complete — ${machineId}** ${urgencyColor}`,
      `Status: **${status.toUpperCase()}** | Urgency: **${urgency.toUpperCase()}**`,
      score != null ? `Anomaly score: **${score.toFixed(4)}**` : null,
      faults.length > 0 ? `Active faults: **${faults.join(', ')}**` : 'No active faults detected.',
      rul != null ? `Estimated RUL: **${rul.toFixed(1)}h**` : null,
      '',
      `**Work Order:**`,
      workOrder,
    ].filter(l => l !== null);
  ...
  const send = useCallback(async (text) => {
    ...
    try {
      ...
      // Append assistant message — loading stays true until useEffect sees it rendered
